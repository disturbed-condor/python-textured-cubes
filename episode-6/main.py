	...
	def on_mouse_motion(self, x, y, delta_x, delta_y):
		if self.mouse_captured:
			...
			self.camera.rotation[0] -= (
				delta_x * sensitivity
			)  # this needs to be negative since turning to the left decreases delta_x while increasing the x rotation angle
...
class Game:
	def __init__(self):
		...
		self.window = Window(
			config=self.config, width=800, height=600, caption="textured cubes & keyboard mouse camera", resizable=True, vsync=False
		)
